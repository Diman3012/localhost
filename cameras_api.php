...
try {
    if ($_SERVER['REQUEST_METHOD'] === 'GET') {
        ...
        $sql = "SELECT c.id, c.ip_camera, c.camera_type_id,
                       ct.description AS camera_type_description
                FROM cameras c
        ...
        // ✅ ВАЖНО: убираем номер — у всех камер только "Камера"
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        if ($action === 'delete') {
            try {
                if ($id) {
                    if ($wid) {
                        $stmt = $pdo->prepare("UPDATE warehouses SET account_camera = NULL WHERE id = :wid AND account_camera = :cid");
                        $stmt->execute([':wid' => $wid, ':cid' => $id]);

                        $stmt = $pdo->prepare("UPDATE warehouses SET camera_state_numbers = NULL WHERE id = :wid AND camera_state_numbers = :cid");
                        $stmt->execute([':wid' => $wid, ':cid' => $id]);
                    } else {
        ...
        if ($ip !== null) {
            $updates[] = 'ip_camera = :ip';
            $params[':ip'] = $ip;
        }

        if ($ctype !== null) {
            $updates[] = 'camera_type_id = :ctype';
            $params[':ctype'] = $ctype;
        }
